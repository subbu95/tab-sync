...
import * as psrConfig from './product-segment-resolution.config'
...
@Component({
  selector: 'eclipse-product-segment-resolution',
  standalone: true,
  imports: [SidePanelModule, FilterdrawerComponent, IconButtonModule, ScreenheaderComponent,
    CommonGridComponent, CommonModule, EclipseAthenaDialogComponent, GridUiComponent, GridComponent],
  templateUrl: './product-segment-resolution.component.html',
  styleUrl: './product-segment-resolution.component.scss'
})
export class ProductSegmentResolutionComponent implements OnInit, OnDestroy, AfterViewInit {
  title = CONSTANTS.DETAILS_TITLE;
  monId!: number;
  detailsPath: string = CONSTANTS.FUP.PSR.STEP_DETAILS_API_PATH;
  ...
  public columns = psrConfig.columns; // Get the grid column names from component config
  public filterFieldDetails!: filterFieldDetailsObject; // Get the filter field details from component config
  public filterDetails = psrConfig.filterDetails; // Get the filter details from component config
  public screenHeaderComponentDetails = psrConfig.screenHeaderComponentDetails; // Get the header details from component config like title & action buttons
  public historyScreenDetails = psrConfig.historyScreenDetails; // Get the history screen details from component config
  public excelExportApiPath = psrConfig.excelExportApiPath;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      psrConfig.filterFieldDetails.dropDown?.forEach(field => {
      ...
      this.filterFieldDetails = psrConfig.filterFieldDetails;
