...
import * as rinConfig from './rule-induction.config'
...
@Component({
  selector: 'eclipse-rule-induction',
  standalone: true,
  imports: [SidePanelModule, FilterdrawerComponent, IconButtonModule, ScreenheaderComponent, CommonGridComponent, CommonModule, EclipseAthenaDialogComponent, GridUiComponent, GridComponent],
  templateUrl: './rule-induction.component.html',
  styleUrl: './rule-induction.component.scss'
})
export class RuleInductionComponent implements OnInit, OnDestroy, AfterViewInit {
  title = CONSTANTS.DETAILS_TITLE;
  monId!: number;
  detailsPath: string = CONSTANTS.FUP.RIN.STEP_DETAILS_API_PATH;
  ...
  public columns = rinConfig.columns; // Get the grid column names from component config
  public filterFieldDetails!: filterFieldDetailsObject; // Get the filter field details from component config
  public filterDetails = rinConfig.filterDetails; // Get the filter details from component config
  public screenHeaderComponentDetails = rinConfig.screenHeaderComponentDetails; // Get the header details from component config like title & action buttons
  public historyScreenDetails = rinConfig.historyScreenDetails; // Get the history screen details from component config
  public excelExportApiPath = rinConfig.excelExportApiPath;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      rinConfig.filterFieldDetails.dropDown?.forEach(field => {
      ...
      this.filterFieldDetails = rinConfig.filterFieldDetails;
