...
import * as phgConfig from './product-hierarchy-generation.config'
...
@Component({
  selector: 'eclipse-product-hierarchy-generation',
  standalone: true,
  ...
  templateUrl: './product-hierarchy-generation.component.html',
  styleUrl: './product-hierarchy-generation.component.scss'
})
export class ProductHierarchyGenerationComponent implements OnInit, OnDestroy, AfterViewInit {
  title = CONSTANTS.DETAILS_TITLE;
  monId!: number;
  detailsPath: string = CONSTANTS.FUP.PHG.STEP_DETAILS_API_PATH;
  ...
  public columns = phgConfig.columns; // Get the grid column names from component config
  public filterFieldDetails!: filterFieldDetailsObject; // Get the filter field details from component config
  public filterDetails = phgConfig.filterDetails; // Get the filter details from component config
  public screenHeaderComponentDetails = phgConfig.screenHeaderComponentDetails; // Get the header details from component config like title & action buttons
  public historyScreenDetails = phgConfig.historyScreenDetails; // Get the history screen details from component config
  public excelExportApiPath = phgConfig.excelExportApiPath;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      phgConfig.filterFieldDetails.dropDown?.forEach(field => {
      ...
      this.filterFieldDetails = phgConfig.filterFieldDetails;
