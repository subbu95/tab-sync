...
import * as prgConfig from './product-mapped-char-rule-generation-by-pos.config'
...
@Component({
  selector: 'eclipse-product-mapped-char-rule-generation-by-pos',
  standalone: true,
  ...
  templateUrl: './product-mapped-char-rule-generation-by-pos.component.html',
  styleUrl: './product-mapped-char-rule-generation-by-pos.component.scss'
})
export class ProductMappedCharRuleGenerationByPosComponent implements OnInit, OnDestroy, AfterViewInit {
  title = CONSTANTS.DETAILS_TITLE;
  monId!: number;
  detailsPath: string = CONSTANTS.FUP.PRG.STEP_DETAILS_API_PATH;
  ...
  public columns = prgConfig.columns; // Get the grid column names from component config
  public filterFieldDetails!: filterFieldDetailsObject; // Get the filter field details from component config
  public filterDetails = prgConfig.filterDetails; // Get the filter details from component config
  public screenHeaderComponentDetails = prgConfig.screenHeaderComponentDetails; // Get the header details from component config like title & action buttons
  public historyScreenDetails = prgConfig.historyScreenDetails; // Get the history screen details from component config
  public excelExportApiPath = prgConfig.excelExportApiPath;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      prgConfig.filterFieldDetails.dropDown?.forEach(field => {
      ...
      this.filterFieldDetails = prgConfig.filterFieldDetails;
