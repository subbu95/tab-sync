...
import * as osrConfig from './product-output-set-resolution.config';
...
@Component({
  selector: 'eclipse-product-output-set-resolution',
  standalone: true,
  ...
  templateUrl: './product-output-set-resolution.component.html',
  styleUrl: './product-output-set-resolution.component.scss'
})
export class ProductOutputSetResolutionComponent implements OnInit, OnDestroy, AfterViewInit {
  title = CONSTANTS.DETAILS_TITLE;
  monId!: number;
  detailsPath: string = CONSTANTS.FUP.OSR.STEP_DETAILS_API_PATH;
  ...
  public columns = osrConfig.columns; // Get the grid column names from component config
  public filterFieldDetails!: filterFieldDetailsObject; // Get the filter field details from component config
  public filterDetails = osrConfig.filterDetails; // Get the filter details from component config
  public screenHeaderComponentDetails = osrConfig.screenHeaderComponentDetails; // Get the header details from component config like title & action buttons
  public historyScreenDetails = osrConfig.historyScreenDetails; // Get the history screen details from component config
  public excelExportApiPath = osrConfig.excelExportApiPath;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      osrConfig.filterFieldDetails.dropDown?.forEach(field => {
      ...
      this.filterFieldDetails = osrConfig.filterFieldDetails;
